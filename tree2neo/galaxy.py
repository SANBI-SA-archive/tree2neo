...
def submit_fasttree_job(api_key, fasta_filename,
                        galaxy_url='http://192.168.2.218',
                        history_name='working_history'):
    gi = get_gi(api_key, galaxy_url)
    if gi is not None:
...
def wait_on_output(api_key, job_id, galaxy_url='http://192.168.2.218'):
    job_state = 'new'
    good_job_states = set(('new', 'queued', 'running'))
    while (job_state != 'ok' and job_state != 'error' and
           job_state in good_job_states):
        print('galaxy job state:', job_state, file=sys.stderr)
        job_state = get_job_state(api_key, job_id, galaxy_url)
        sleep(1)
    ...
    if job_state == 'ok':
        ...
        if gi is not None:
            ...
            # fasttree generates a nhx (newick tree) format output
            # called output_tree
            if 'exit_code' in job_result and job_result['exit_code'] == 0:
                print("job completed with exit code 0, all good")
                if ('outputs' in job_result and 'output_tree' in
                        job_result['outputs']):
                    tree_output_id = \
                        job_result['outputs']['output_tree']['id']
...
def delete_history(api_key, history_id, galaxy_url='http://192.168.2.218'):
    gi = get_gi(api_key, galaxy_url)
    if gi is not None:
